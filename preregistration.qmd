...
There may be different requirements for each type of research and the [Equator Network](https://www.equator-network.org/) have provided [guidance](https://www.equator-network.org/?post_type=eq_guidelines&eq_guidelines_study_design=study-protocols&eq_guidelines_clinical_specialty=0&eq_guidelines_report_section=0&s=+) on what to include in protocols for a selection of different designs. To date, this includes trials, intervention detail, systematic reviews, tomography studies, intervention development studies, and core outcome sets.
You can also publish the protocol as a peer-reviewed journal article, but this should be considered in addition to the registrations below given the time the peer-review process takes. If your chosen journal facilitates this (see [Sherpa Romeo](https://v2.sherpa.ac.uk/romeo/) you could deposit the pre-print (i.e., the manuscript submitted for publication) at a pre-print server such as [PsyArXiv](https://psyarxiv.com/).  
...
-   Example: [Reid and Dessing (2016)](https://osf.io/sqh36)
